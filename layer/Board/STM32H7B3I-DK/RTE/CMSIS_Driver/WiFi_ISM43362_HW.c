...
void WiFi_ISM43362_Pin_RSTN (uint8_t rstn) {
  HAL_GPIO_WritePin(GPIOI, GPIO_PIN_1, rstn ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
...
void WiFi_ISM43362_Pin_SSN (uint8_t ssn) {
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, ssn ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
...
uint8_t WiFi_ISM43362_Pin_DATARDY (void) {
  return (HAL_GPIO_ReadPin(GPIOI, GPIO_PIN_5) == GPIO_PIN_SET);
}
