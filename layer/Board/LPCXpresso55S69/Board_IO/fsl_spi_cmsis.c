...
static uint32_t SPI_InterruptGetCount(cmsis_spi_interrupt_driver_state_t *spi)
{
    ...
    if (spi->flags & SPI_FLAG_MASTER)
    {
        if (spi->handle->masterHandle.txData != 0U) {
          cnt = spi->handle->masterHandle.totalByteCount - spi->handle->masterHandle.txRemainingBytes;
        } else {
    ...
    {
        if (spi->handle->slaveHandle.txData != 0U) {
          cnt = spi->handle->slaveHandle.totalByteCount - spi->handle->slaveHandle.txRemainingBytes;
        } else {
